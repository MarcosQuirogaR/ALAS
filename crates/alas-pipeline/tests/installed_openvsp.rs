...
fn installed_openvsp_materializes_and_validates_the_native_project() {
    ...
    if std::env::var_os("ALAS_REQUIRE_NATIVE_PREVIEW").is_some() {
        assert!(export.preview_available, "native screenshot required: {:?}", export.preview_error);
        assert!(export.preview_error.is_none());
        let capture_stdout = fs::read_to_string(export.script_path.with_extension("preview.stdout.txt")).unwrap();

...
//!
//! The three source PNGs live at the workspace root so the desktop shell and
//! the release package consume the same artwork.  This module keeps the
//! loading paths together and gives the native window a square, transparent
//! canvas for the wide three-stripe mark instead of allowing an operating
//! system icon API to distort it.

use egui::IconData;

const APP_LOGO_PNG: &[u8] = include_bytes!("../../../app_logo.png");
const APP_TEXT_LOGO_PNG: &[u8] = include_bytes!("../../../app_text_logo.png");
const APP_BRANDING_PNG: &[u8] = include_bytes!("../../../app_branding.png");
...
/// Embedded composite artwork for the native splash and About surfaces.
pub(crate) fn composite_image(ctx: &egui::Context) -> Option<egui::Image<'static>> {
    Some(egui::Image::from_texture(&cached_texture(
        ctx,
        "alas-native-composite-branding",
        APP_BRANDING_PNG,
    )?))
}
...
#[cfg(test)]
mod tests {
    use super::{icon_data, APP_LOGO_PNG};

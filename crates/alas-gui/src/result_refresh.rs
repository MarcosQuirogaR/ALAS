...
impl AppState {
    ...
    pub(crate) fn verify_final_result_figures(&mut self) {
        ...
        let Some(result) = &self.pipeline_result else { return };
        let config = result.config.clone();
        let expected_patran = result.structural_result.as_ref()
            .and_then(|structure| structure.patran.as_ref())
            .map(|patran| patran.png_paths.iter()
                .map(|(_, path)| path.to_string_lossy().replace('\\', "/")).collect::<BTreeSet<_>>())
            .unwrap_or_default();
        ...
        for figure in alas_report::RESULT_FIGURES {
            let key = format!("run={};solver={:?};theme={theme};language={language};figure={}",
            ...
            let Some(scene) = self.cached_result_figure_with_camera(
                &key, figure.id, &config, &theme, camera,
            ) else {
                unavailable += 1;
                continue;
            };
        ...
        for path in expected_patran.difference(&images) {
            issues.push(format!("Patran produced an image missing from the gallery: {path}"));
        }
        for path in images {
            if std::fs::read(&path).ok()
                .and_then(|bytes| eframe::icon_data::from_png_bytes(&bytes).ok()).is_none() {
                issues.push(format!("Final figure image is missing or cannot be decoded: {path}"));
            }
        }
        ...
        for issue in issues { self.log(issue, LogKind::Warn); }
    }
}
...
mod tests {
    ...
    fn new_result_data_evicts_unavailable_and_old_scenes_without_resetting_views() {
        ...
        state.result_figure_cache.insert("run=1;figure=structures_patran".into(), None);
        state.result_figure_cache.insert("run=1;figure=openvsp_cad_preview".into(),
            Some(Arc::new(Scene::new(10.0, 10.0, None))));
        state.view_states.insert("keep-camera".into(), Default::default());
    ...
    fn completed_result() -> alas_pipeline::PipelineResult {
        ...
        alas_pipeline::DesignPipeline::new(config).run(
            &alas_pipeline::PipelineOptions {
                optimize: false, compare_baseline: false, quiet: true,
                ..Default::default()
    ...
    fn final_check_replaces_early_patran_status_with_completed_image_scene() {
        ...
        let key = format!("run=0;solver={:?};theme={theme};language={};figure=structures_patran",
            state.selected_solver_view, alas_i18n::get_language());
        let early = state.cached_result_figure(&key, "structures_patran", &config, &theme).unwrap();
        assert!(!early.elements.iter().any(|e| matches!(e, SceneElement::Image { .. })));
        let path = std::env::temp_dir().join(format!("alas-patran-refresh-{}.png", std::process::id()));
        let pixels = alas_viz::raster::render_scene_png(&Scene::new(4.0, 4.0,
            Some(alas_report::scene::Color::rgb(0, 100, 200)))).unwrap();
        std::fs::write(&path, pixels).unwrap();
        state.pipeline_result.as_mut().unwrap().structural_result = Some(
            alas_pipeline::structural::StructuralAnalysisResult {
                status: "ok".into(),
                patran: Some(alas_pipeline::structural::PatranExportResult {
                    status: "ok".into(), png_paths: vec![("Level".into(), path.clone())],
                    ..Default::default()
                }), ..Default::default()
            });
        ...
        let final_scene = state.result_figure_cache.get(&key).unwrap().as_ref().unwrap();
    ...
    fn retained_openvsp_mesh_replaces_early_status_and_renders_without_graphics_build() {
        let path = std::path::PathBuf::from(std::env::var_os("ALAS_CAD_MESH").expect("retained mesh"));
        let mut state = AppState::default();
        state.pipeline_result = Some(completed_result());
        ...
        let scene = alas_report::families::geometry::figure_openvsp_cad_preview(Some(export), Some("dark"));
        assert!(scene.elements.iter().any(|element| matches!(element, SceneElement::Polygon { .. })));
        let png = alas_viz::raster::render_scene_png(&scene).expect("native mesh rasterizes");
        let output = std::path::PathBuf::from(std::env::var_os("ALAS_CAD_PREVIEW_OUTPUT").expect("preview destination"));
